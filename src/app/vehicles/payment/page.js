...
function PaymentContent() {
  ...
  const price = selectedVehicle
    ? (selectedVehicle.discountPrice ?? selectedVehicle.basePrice)
    : 0;

  // Find next upgrade vehicle
  const vehicleIdx = VEHICLES.findIndex((v) => v.id === vehicle);
  const upgradeVehicle =
    vehicleIdx >= 0 && vehicleIdx < VEHICLES.length - 1
      ? VEHICLES[vehicleIdx + 1]
      : null;
  const upgradePrice = upgradeVehicle
    ? (upgradeVehicle.discountPrice ?? upgradeVehicle.basePrice)
    : 0;
              ...
              <div className="space-y-3 mb-4">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">
                    Outward · {selectedVehicle?.name ?? "Vehicle"}
                  </span>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-start gap-3 p-4 rounded-2xl bg-[#B8960C]/5 border border-[#B8960C]/20">
                ...
                <p className="text-sm text-gray-700">
                  ...
                  You can cancel for free before{" "}
                  <span className="font-semibold">{cancelDate}</span> and get a
                  full refund.
                </p>
              </div>
            </div>
